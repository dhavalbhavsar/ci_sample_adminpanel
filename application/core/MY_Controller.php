...
class MY_Controller extends MX_Controller {
	...
	private function _setup()
	{
		$site_config = $this->config->item('site');
                $idData = $this->session->userdata('identity');
                $defaultData = $this->ion_auth->get_group_default_by_identity()->result();
                $site_arr = array();
                if($idData != 'webmaster' && !empty($idData)){
                    $site_arr['panel'] = array($idData);
                    $idAllData = $this->ion_auth->get_group_permission_by_identity($idData)->result();
                    
                    $idAllData = array_merge($idAllData,$defaultData);
                    // restrict pages
                    foreach ($idAllData as $key=>$va){
                        $site_arr[$va->controller.'/'.$va->action] = array($idData);
                        if($va->module != $this->mModule && $va->controller != $this->mCtrler && $va->action != $this->mAction){
