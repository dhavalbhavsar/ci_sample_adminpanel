...
class Ion_auth_model extends CI_Model {
     ...
     * @author Mathew
     * */
    public function identity_check_edit($identity = '', $user_id) {
        $this->trigger_events('identity_check');
     ...
     * @author Mathew
     * */
    public function register_new($identity, $password, $email, $additional_data = array(), $groups, $status) {
     ...
     * @author Mathew
     * */
    public function update_group_action($identity, $password, $email, $additional_data = array(), $groups, $status) {
     ...
     * @author Mathew
     * */
    public function register_update($identity, $additional_data = array(), $groups, $status, $user_id) {
        $this->trigger_events('pre_register');

        $manual_activation = $this->config->item('manual_activation', 'ion_auth');

        if ($this->identity_check_edit($identity, $user_id)) {
            $this->set_error('account_creation_duplicate_identity');
            return FALSE;
        } elseif (!$this->config->item('default_group', 'ion_auth') && empty($groups)) {
            $this->set_error('account_creation_missing_default_group');
        ...
        $rowAdminGroup = $this->db->get_where('admin_users_groups', array('user_id' => $user_id))->row();

        $adminUserGroupId = $rowAdminGroup->group_id;
        // filter out any data passed that doesnt have a matching column in the users table
        // and merge the set user data and the additional data
        $user_data = array_merge($this->_filter_data($this->tables['users'], $additional_data), $data);
        ...
        // add in groups array if it doesn't exits and stop adding into default group if default group ids are set
        $this->remove_from_group($adminUserGroupId, $user_id);
        $this->add_to_group($groups, $user_id);
     ...
     * @author Ben Edmunds
     * */
    public function get_users_group_action($group_id) {
        $this->trigger_events('get_users_group_action');

        // if no id was passed use the current users id

        $select =  $this->db->select('admin_users_action.id,admin_users_action.action,IF(admin_group_action.id IS NULL,"false","true") as selected')
                        ->join('admin_group_action', 'admin_group_action.admin_users_action_id	= admin_users_action.id AND admin_group_action.admin_groups_id = '.$group_id,'left')
                        ->where('admin_users_action.by_default', 'false')
                        ->get('admin_users_action');
     ...
     * @author Ben Edmunds
     * */
    public function add_users_group_permision($data){
        $this->db->insert('admin_group_action', $data);
        return ;
    }
     ...
     * @author Ben Edmunds
     * */
    public function remove_users_group_permision($group_id){
        $this->db->delete('admin_group_action', array('admin_groups_id' => $group_id));
        return ;
    }
     ...
     * @author Ben Edmunds
     * */
    public function get_group_permission_by_identity($group_name){
        $this->db->select('admin_groups.name,admin_users_action.*');
        $this->db->from('admin_groups');
        $this->db->join('admin_group_action', 'admin_groups.id = admin_group_action.admin_groups_id', 'inner');
        $this->db->join('admin_users_action', 'admin_group_action.admin_users_action_id = admin_users_action.id', 'inner');
        $this->db->where('admin_groups.name', $group_name);
        return $this->db->get();
     ...
     * @author Ben Edmunds
     * */
    public function get_group_default_by_identity(){
        $this->db->select('admin_users_action.*');
        $this->db->from('admin_users_action');
        $this->db->where('admin_users_action.by_default', 'true');
        return $this->db->get();
    ...
    public function get_user_data($user_id) {
        $this->db->select('admin_users.*,admin_users_groups.group_id');
        $this->db->from('admin_users');
        $this->db->join('admin_users_groups', 'admin_users.id = admin_users_groups.user_id', 'inner');
    ...
    public function get_user_group($group_id) {
        $this->db->select('admin_users.*,admin_users_groups.group_id');
        $this->db->from('admin_users');
        $this->db->join('admin_users_groups', 'admin_users.id = admin_users_groups.user_id', 'right');
        $this->db->where('admin_users_groups.group_id', $group_id);
        return $this->db->get();
    }
     ...
     * @author aditya menon
     * */
    public function delete_admin_group($group_id = FALSE) {
        // bail if mandatory param not set
        if (!$group_id || empty($group_id)) {
            return FALSE;
        }
        $group = $this->group($group_id)->row();
        $checkGroupUse = $this->get_user_group($group_id)->result_array();
        if(!empty($checkGroupUse)){
            $this->trigger_events(array('post_delete_group', 'post_delete_group_notallowed'));
            $this->set_error(sprintf(lang('group_delete_user_use'), $group->name));
            return FALSE;
        }
        ...
        // remove all admin group from this group
        $this->db->delete($this->tables['admin_users_groups'], array($this->join['admin_groups'] => $group_id));
        // remove the admin group itself
        $this->db->delete($this->tables['admin_groups'], array('id' => $group_id));
     ...
     * @author Ben Edmunds
     * */
    public function errors($flag = true) {
        if($flag){
            $_output = '';
